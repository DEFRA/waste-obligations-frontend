...
describe('prnSingleController', () => {
  test('renders the PRN view with the organisation, PRN and regulator details', async () => {
    ...
    expect(h.view).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        year: 2026,
        prn,
        backLink: `/organisations/producer/${organisationId}/prns`,
  ...
  test('is configured as a GET route under /organisations/producer/{organisationId}/prns/{prnId}', () => {
    expect(prnSingleController.method).toBe('GET')
    expect(prnSingleController.path).toBe(
      '/organisations/producer/{organisationId}/prns/{prnId}'
    )
  })
