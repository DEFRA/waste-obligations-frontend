...
describe('prnsListController', () => {
    ...
    expect(model.prnsViewModel.rows).toEqual([
      {
        ...
        view: {
          html: `<a class="govuk-link" href="/organisations/producer/${organisationId}/prns/prn-1?year=2026">View</a>`
        }
      }
    ])
  })
  ...
  test('is configured as a GET route under /organisations/producer/{organisationId}/prns', () => {
    expect(prnsListController.method).toBe('GET')
    expect(prnsListController.path).toBe(
