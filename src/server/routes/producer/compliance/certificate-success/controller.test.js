...
describe('certificateSuccessController', () => {
  test('renders success page from compliance declaration API response', async () => {
    ...
    expect(h.view).toHaveBeenCalledWith(
      'producer/compliance/certificate-success/index',
      expect.objectContaining({
        ...
        certificateViewHref: `/producer/b6f76437-65b6-4ed2-a7d5-c50e9af76201/compliance/certificate/${complianceDeclarationId}`
      })
    )
  ...
  test('prefixes the certificate view link for a reverse proxy', async () => {
    ...
    expect(model.certificateViewHref).toBe(
      `/manage-recycling-obligations/producer/${request.params.organisationId}/compliance/certificate/${complianceDeclarationId}`
    )
  })
