...
describe('certificateSubmitPostController', () => {
  ...
  test('redirects to success with met status and posts compliance declaration', async () => {
    ...
    expect(redirect).toHaveBeenCalledWith(
      `/producer/${organisationId}/compliance/certificate/${createdComplianceDeclarationId}/success`
    )
    expect(result).toBe('REDIRECT')
  })
  ...
  test('redirects in Welsh when lang=cy', async () => {
    ...
    expect(redirect).toHaveBeenCalledWith(
      `/producer/${organisationId}/compliance/certificate/${createdComplianceDeclarationId}/success?lang=cy`
    )
  })
