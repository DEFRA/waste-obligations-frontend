...
describe('auth controllers', () => {
  ...
  describe('signOutController', () => {
    ...
    test('uses local signed-out URL when referer matches packaging sign-out origin', () => {
      configGetMock.mockImplementation((key) => {
        if (key === 'auth.azureAdB2c') {
          return {
            instance: 'https://tenant.b2clogin.com',
      ...
      expect(h.redirect).toHaveBeenCalledWith(
        expect.stringContaining(
          'post_logout_redirect_uri=https%3A%2F%2Flocalhost%3A8010%2Fsigned-out'
        )
      )
      expect(h.redirect).not.toHaveBeenCalledWith(
        expect.stringContaining('localhost%3A7084')
      )
    })
